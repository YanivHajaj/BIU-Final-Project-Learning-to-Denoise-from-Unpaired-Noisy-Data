...
def generate(args):
    ...
    model.load_state_dict(torch.load('./experiments/exp{}/checkpoints/{}epochs.pth'.format(args.exp_num, args.n_epochs),
                                     map_location=device))
    model.eval()
    ...
    for index, clean255 in enumerate(imgs):
        ...
        if noise_type == 'gauss':
            ...
            if not opt.noisy_input:
                ...
                noisy_numpy = clean_numpy
            noisier_numpy_single = noisy_numpy + (
                        np.random.poisson(clean_numpy * 255. * noise_intensity) / noise_intensity / 255. - clean_numpy)
        else:
        ...
        for i in range(args.aver_num):
            if noise_type == 'gauss':
                ...
                noisier_numpy = noisy_numpy + (np.random.poisson(
                    clean_numpy * 255. * noise_intensity) / noise_intensity / 255. - clean_numpy)
            else:
                raise NotImplementedError('wrong type of noise')
        ...
        if args.normalize:
            ...
            noisier = denorm(noisier_numpy_single, mean=args.mean,
                             std=args.std)  # Add denormalization for noisier (the single noisier of the predict, not overlap)

        prediction, overlap_mean, overlap_median, overlap_trimmed = tensor_to_numpy(prediction), tensor_to_numpy(
            overlap_mean), tensor_to_numpy(overlap_median), tensor_to_numpy(overlap_trimmed)
        prediction_numpy, overlap_mean_numpy, overlap_median_numpy, overlap_trimmed_numpy = np.squeeze(
            prediction), np.squeeze(overlap_mean), np.squeeze(overlap_median), np.squeeze(overlap_trimmed)
        ...
        print(
            '{}th image | PSNR: noisy:{:.3f}, prediction:{:.3f}, overlap_mean:{:.3f}, overlap_median:{:.3f}, overlap_trimmed_mean:{:.3f}, noisier:{:.3f} | SSIM: noisy:{:.3f}, prediction:{:.3f}, overlap_mean:{:.3f}, overlap_median:{:.3f}, overlap_trimmed_mean:{:.3f}, noisier:{:.3f}'.format(
        ...
        file_name = os.path.splitext(os.path.basename(img_paths[index]))[
            0]  # Extracts the base file name without extension
        ...
        if index <= 10:
            ...
            cv2.imwrite(os.path.join(save_dir, '{}th_noisier.png'.format(index + 1)),
                        sample_noisier)  # Save noisier image

    # Total PSNR, SSIM
    print(
        '{} Average PSNR | noisy:{:.3f}, prediction:{:.3f}, overlap_mean:{:.3f}, overlap_median:{:.3f}, overlap_trimmed_mean:{:.3f}'.format(
    ...
    print(
        '{} Average SSIM | noisy:{:.3f},  prediction:{:.3f}, overlap_mean:{:.3f}, overlap_median:{:.3f}, overlap_trimmed_mean:{:.3f}'.format(
